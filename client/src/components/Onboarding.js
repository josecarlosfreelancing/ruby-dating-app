...
function Onboarding({ user, showAuth, isEditingProfile, setUser }) {
  const [profile_photo_type, set_profile_photo_type] = React.useState('url')

  const navigate = useNavigate()
  const formik = useFormik({
    enableReinitialize: true,
    initialValues: {
      profile_photo_type: 'url',
  ...
  function handleChangeFile (e) {
    if (e.target.files[0]) {
      ...
      const storageRef = ref(storage, `${timestamp.toString()}`);
      uploadBytes(storageRef, e.target.files[0])
        .then(snapshot => {
          getDownloadURL(storageRef)
            .then(url => {
              // const extension = e.target.files[0].name.split('.').pop();
              formik.setFieldValue('url1', url);
            })
        })
    }
  }

  function handleProfilePhotoType (type) {
    set_profile_photo_type(type);
  }
            ...
            <div className="profile-photo">
              ...
              <div className="profile-photo-container">
                <input 
                    ...
                    name="profile_photo_type" 
                    type="radio" 
                    value="url"
                    // onBlur={formik.handleBlur}
                    onChange={() => handleProfilePhotoType('url')}
                    checked={profile_photo_type === 'url'}
                />
                <label htmlFor="profile-photo-url">URL</label>
                <input 
                    ...
                    value="file"
                    // onBlur={formik.handleBlur}
                    onChange={() => handleProfilePhotoType('file')}
                    checked={profile_photo_type === 'file'}
                />
                <label htmlFor="profile-photo-file">File</label>                
              </div>  
            </div>
            <input
              id="url"
              ...
              style={{
                display: profile_photo_type === 'url' ? 'block' : 'none'
              }}
            />
            <input
              id="file"
              ...
              style={{
                display: profile_photo_type === 'file' ? 'block' : 'none'
              }}
            />
